...
const fillForm = () => {
  const nameInput = screen.getByLabelText('Name') as HTMLInputElement;
    ...
    fireEvent.change(birthdayInput, { target: { value: users[0].birthday.toString() } });
    ...
    const privacyInput = screen.getByLabelText('Agree to Privacy Rules') as HTMLInputElement;
...
describe('Form component', () => {
  ...
  test('inputs are rendered and their values are submitted to a card', () => {
    ...
    expect(privacyInCard[0].textContent).toEqual(`Consent to Privacy Rules: ${users[0].agreedToPrivacyRules.toString()}`);
